...
刪除書籍後，`ContentView` 清理同 UUID 的獨立發布狀態；清理失敗時顯示錯誤。備份與還原涵蓋狀態檔，舊備份缺檔時狀態預設草稿。隔離資料 GUI 刪除及還原驗收待完成。

## V10.2 發布數據

每日編輯淨字數按 Book UUID 保存在 `Sailune/Writing Stats.json`，不改 V5 schema；永久刪書後清除對應記錄，完整備份及還原涵蓋 sidecar。舊備份沒有統計檔時會清除目前統計。正文保存與統計檔寫入是兩次交易：若前者成功而後者失敗，正文保留，當次啟動停用受影響書籍統計並顯示錯誤；若程式在錯誤後關閉，sidecar 無法持久記錄失敗旗標，重開後可能留下未被辨識的少記風險。讀者指標沒有資料來源，畫面維持空白。隔離資料 GUI、故障注入及備份 rollback 驗收待完成。
