...
- 共用 UI 元件新增／修改時，驗收每個正式 variant 的正常、hover／focus、disabled、錯誤及適用的 loading 狀態；核對淺／深外觀、鍵盤操作與 accessibility label。
- 遷移共用按鈕、欄位、編輯器、圖標或文案資源時，盤點全部呼叫點並抽查各類使用情境；核對同語意畫面一致、列明例外，確認沒有因共用化改變輸入、組字、Undo、保存或刪除行為。
- UI 命中範圍、遮罩、疊層或競爭手勢有變更時，使用 GUI 人工驗收實際命中區、背景穿透、取消路徑及滑鼠／鍵盤操作；單元測試不能取代這項驗收。
- 新增或抽取 Store／Coordinator action 時，驗證單一呼叫入口、成功／失敗分支、重複呼叫或重試語意，以及呼叫端顯示的結果；依工作單中使用者核定的錯誤政策驗收，不套用未批准的通用行為。
- 純文件工作只驗文件差異與格式，不因文件內容提及測試就執行建置或測試；工作單須明示未執行的程式驗證範圍。
