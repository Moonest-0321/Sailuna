...
- **實作**：`EditorWorkspaceView.sectionRow` 將此前綴從共用中文節次名稱改為當列卷內序號；更新 `docs/spec-editor.md` 與 V10.1 工作紀錄。
- **驗證**：`git diff --check` 通過；依工作規則，本次沒有執行測試。
- **工作樹邊界**：保留工作樹中已有的 V10.2 修改及使用者原有 `Sailune/Localizable.xcstrings` 修改；本修正只改編輯器目錄列及其規格／紀錄。
...
# 2026-09-26 V10.2 發布書籍數據頁 implementation checkpoint

- **已決定**：R／U／I 已核准。每日編輯字數採有號淨變化；點書名進頁，發布狀態操作獨立；日期為本地今日加前六日。reader 資料缺來源時留白。
- **實作**：新增 `BookWritingStatsStore` JSON sidecar 與 `BookAnalyticsView`；發布頁書名導覽及返回；編輯器正文成功保存／flush 去重及搜尋取代計入；備份、舊備份清除、rollback 與永久刪書清理接線。V5 schema 不變。自動同步、修復、匯入與刪除不計入編輯統計。
- **驗證**：無簽章 Debug build 通過；受影響 Swift parse、`git diff --check` 通過。統計 Store 4 項、備份 round-trip 及舊備份 2 項 XCTest 通過。保留原使用者修改 `Sailune/Localizable.xcstrings`。
- **GUI**：隔離 Debug clone `/private/tmp/SailuneV102Smoke/SailuneV102Smoke.app` 以 `/private/tmp/SailuneV102Smoke/Isolated.store` 建書成功、回到發布列表並看見書名與獨立「發布」按鈕；點擊書名後 CUA 回報 native pipe closed，重置後仍不能讀取數據頁。頁面進入／返回、圖表實際視覺呈現仍未確認，正式作者資料未操作。
- **已知風險**：正文與 sidecar 無共同原子交易。sidecar 寫入失敗時當次只隱藏受影響書籍統計；若程式在錯誤後關閉，失敗旗標不能持久化，重開可能不知少記。詳見規格、備份及一致性文件。
- **唯一下一步**：在 CUA 可正常綁定隔離 Debug App 的環境確認點書名進數據頁、返回、空圖表與狀態操作獨立；不得改用正式資料。新聊天讀取：本文件、`docs/work-items/current.md`、`docs/work-items/v10.2-publishing-analytics.md`、`docs/spec-publishing-analytics.md`、`docs/project-status.md`。
