...
>
> 唯一下一步：請使用者確認或修正修訂後的 UI 提案；U 批准後再列 I 實作與測試計畫。

## 2026-09-21 V6.3 角色設定集圖示化（R approved；U 待確認）
...
- 使用者追加：正文引用過多時限制在該欄位內捲動並顯示捲軸；「加入」按鈕放在正文引用區最上方。追查全流程後確認引用是從正文連結／名稱掃描計算；目前可由選取文字後連結角色，或輸入角色呼叫插入連結名稱。故 UI 提案讓按鈕沿用同一套角色連結語意：連結有效選取，否則於目前節次游標插入真名連結；不建立手動章節關係或新增 schema。這個 code-based 提案待 U 確認；尚未修改功能程式或資料。
...
- 第一個下一步：請使用者確認或修正修訂後的 UI 提案；新聊天最小讀取集合為 `docs/work-items/current.md`、`docs/handoffs/current.md` 與 `docs/project-status.md`。
